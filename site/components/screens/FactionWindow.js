...
export default function FactionWindow({
  position,
  isDragging,
  isActive,
  handleMouseDown,
  ...
  return (
    <>
      ...
      >
        ...
        >
          <p>
            ooo... you have {availableTickets} juice tickets to invite your
            friends to join you on this adventure
          </p>
          <div style={{ display: "flex", flexDirection: "row", gap: 16 }}>
            {tickets.map((ticket) => {
              console.log(
                `Ticket ID: ${ticket.id}, Image Path: ${ticket.image}`,
              );
              return (
                <div
                  key={ticket.id}
                  style={{
                    ...
                    animation: ticket.used
                      ? "none"
                      : "ticketShake 1.2s linear infinite",
