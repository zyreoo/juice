...
export default function ShareSuccessPanel() {
  ...
  const getCursorStyle = (x, y) => {
    ...
    const nearBottom =
      Math.abs(y - (cropBox.y + cropBox.height)) < edgeThreshold;
    ...
    if (cropBox.isResizing) {
      setCropBox((prev) => {
        ...
        if (corner.includes("s")) {
          ...
          if (
            proposedHeight >= 50 &&
            prev.y + proposedHeight <= container.clientHeight
          ) {
            newHeight = proposedHeight;
          }
        }
        ...
        if (corner.includes("e")) {
          ...
          if (
            proposedWidth >= 50 &&
            prev.x + proposedWidth <= container.clientWidth
          ) {
            newWidth = proposedWidth;
          }
        }
      ...
      setCropBox((prev) => {
        const newX = Math.max(
          0,
          Math.min(prev.x + dx, container.clientWidth - prev.width),
        );
        const newY = Math.max(
          0,
          Math.min(prev.y + dy, container.clientHeight - prev.height),
        );
        return {
          ...prev,
          x: newX,
  ...
  return (
    ...
    >
      <p style={{ color: "rgba(0, 0, 0, 0.8)", margin: "0 0 8px 0" }}>
        {kudosSent
          ? 'Shared your "OMG IT WORKS" moment! Kudos coming your way...'
          : "Share your success!"}
      </p>
      {(isSharing || showingPreview) && (
        <div
          ...
          onMouseDown={
            !showingPreview && !isRecording ? handleMouseDown : undefined
          }
          onMouseMove={
            !showingPreview && !isRecording ? handleMouseMove : undefined
          }
        >
          {!showingPreview && (
            <video
