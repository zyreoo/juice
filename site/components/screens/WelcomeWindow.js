...
export default function WelcomeWindow({
  ...
  const options = [
    isLoggedIn ? "Start Juicing" : "Join Jam",
    "Learn More",
    "Exit",
  ];
  ...
  useEffect(() => {
    ...
    if (audioRef.current) {
      ...
      const handleTimeUpdate = () => {
        ...
        if (
          fadeOutStartTimeRef.current &&
          audio.currentTime >= fadeOutStartTimeRef.current
        ) {
          ...
          {options.map((option, index) => (
            ...
            >
              <span
                style={{
                  animation:
                    selectedOption === index && !executedOptions.has(option)
